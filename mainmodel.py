import edge_tts   #Microsoft Edge browser ka Text-to-Speech engine
import asyncio   #multiple tasks ek saath chalane ke liye
...
import os   #File check karna, path banana, etc.
import speech_recognition as sr
import google.generativeai as genai
import firebase_admin
from firebase_admin import credentials, firestore
import socketio #WebSockets ka Python implementation.   Real-time communication (e.g., frontend se caption bhejna, command receive karna).
from time import time  #Execution ka duration check karna, timestamp generate karna.
import subprocess# Python se external command ya scripts run karna.
import re   #User ke input me specific patterns dhundhna   
import random #Random numbers ya elements select karna.
...
async def edge_speak(text):
    ...
    subprocess.run(["mpg123", audio_file])  # Play the audio file using mpg123
...
@sio.event
def disconnect():
    print("âŒ Disconnected from Socket.IO server")
...
def send_images_to_gemini(image_folder="newplant"):
...
# ------------------ ðŸŒ± Crop-specific Data (Firestore) ------------------
...
def query_vegetable_data(query):
    ...
    # Respond based on query keywords
